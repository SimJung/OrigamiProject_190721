...
public class PolygonApplyMeshCut : MonoBehaviour
{
    ...
    private static float lwide, rwide;

    public static GameObject[] Cut(GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Quaternion angle)
    {
        Debug.Log("Cut Start");
        if (victim == null)
        {
        ...
        GameObject leftSideObj = new GameObject("leftSide", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider), typeof(Polygon));
        Vector3 l_mp = getLeftMidPoint();
        l_mp.x *= victim.transform.localScale.x;
        l_mp.y *= victim.transform.localScale.y;
        l_mp.z *= victim.transform.localScale.z;
        leftSideObj.transform.position = l_mp;
        leftSideObj.transform.localPosition = victim_transform.position;

        leftSideObj.transform.rotation = victim_transform.rotation;
        leftSideObj.transform.localScale = victim_transform.localScale;
        leftSideObj.GetComponent<MeshFilter>().mesh = left_HalfMesh;
        leftSideObj.GetComponent<MeshCollider>().sharedMesh = left_HalfMesh;


        GameObject rightSideObj = new GameObject("rightSide", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider), typeof(Polygon));
        Vector3 r_mp = getRightMidPoint();
        r_mp.x *= victim.transform.localScale.x;
        r_mp.y *= victim.transform.localScale.y;
        r_mp.z *= victim.transform.localScale.z;
        rightSideObj.transform.position = r_mp;
        rightSideObj.transform.localPosition = victim_transform.position;

        rightSideObj.transform.rotation = victim_transform.rotation;
        rightSideObj.transform.localScale = victim_transform.localScale;
        ...
        if (rwide >= lwide)
        {
            leftSideObj.GetComponent<Polygon>().HT = victim.GetComponent<Polygon>().HT ? false : true;
            leftSideObj.GetComponent<Polygon>().createShader(leftSideObj);
            leftSideObj.transform.position += new Vector3(0, 0.0001f, 0);
            leftSideObj.transform.rotation = angle;
            return new GameObject[] { rightSideObj, leftSideObj };
        }
            
        else
        {
            rightSideObj.GetComponent<Polygon>().HT = victim.GetComponent<Polygon>().HT ? false : true;
            rightSideObj.GetComponent<Polygon>().createShader(rightSideObj);
            rightSideObj.transform.position += new Vector3(0, 0.0001f, 0);
            rightSideObj.transform.rotation = angle;
            return new GameObject[] { leftSideObj, rightSideObj };
        }
    }


    public static GameObject[] ChildCut(GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Quaternion angle)
    {
        Debug.Log("Cut Start");
        if (victim == null)
        {
        ...
        GameObject emptyLobj = new GameObject("leftSideObj", typeof(Polygon));
        GameObject emptyRobj = new GameObject("rightSideObj", typeof(Polygon));
        emptyLobj.transform.position = Vector3.zero;
        emptyRobj.transform.position = Vector3.zero;

        GameObject leftSideObj = new GameObject("leftSide", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider));
        Vector3 l_mp = getLeftMidPoint();
        l_mp.x *= victim.transform.localScale.x;
        l_mp.y *= victim.transform.localScale.y;
        l_mp.z *= victim.transform.localScale.z;
        ...
        //leftSideObj.transform.position = -l_mp;
        //emptyLobj.transform.position = victim_transform.position + l_mp;
        ...
        leftSideObj.GetComponent<MeshFilter>().mesh = left_HalfMesh;
        leftSideObj.GetComponent<MeshCollider>().sharedMesh = left_HalfMesh;
        leftSideObj.transform.SetParent(emptyLobj.transform);

        GameObject rightSideObj = new GameObject("rightSide", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider));
        Vector3 r_mp = getRightMidPoint();
        r_mp.x *= victim.transform.localScale.x;
        r_mp.y *= victim.transform.localScale.y;
        r_mp.z *= victim.transform.localScale.z;
        ...
        //rightSideObj.transform.position = -r_mp;
        //emptyRobj.transform.position = victim_transform.position + r_mp;
        ...
        rightSideObj.GetComponent<MeshFilter>().mesh = right_HalfMesh;
        rightSideObj.GetComponent<MeshCollider>().sharedMesh = right_HalfMesh;
        rightSideObj.transform.SetParent(emptyRobj.transform);

        leftSideObj.GetComponent<MeshRenderer>().materials = mats;
        rightSideObj.GetComponent<MeshRenderer>().materials = mats;

        Destroy(victim.transform.parent.gameObject);

        Debug.Log("Cut End");
        if (rwide >= lwide)
        {
            emptyLobj.GetComponent<Polygon>().HT = victim.transform.parent.GetComponent<Polygon>().HT ? false : true;
            emptyLobj.GetComponent<Polygon>().createShader(emptyLobj);
            leftSideObj.transform.position += new Vector3(0, 0.0001f, 0);
            leftSideObj.transform.rotation = angle;
            return new GameObject[] { emptyRobj, emptyLobj };
        }

        else
        {
            emptyRobj.GetComponent<Polygon>().HT = victim.transform.parent.GetComponent<Polygon>().HT ? false : true;
            emptyRobj.GetComponent<Polygon>().createShader(emptyRobj);
            rightSideObj.transform.position += new Vector3(0, 0.0001f, 0);
            rightSideObj.transform.rotation = angle;
            return new GameObject[] { emptyLobj, emptyRobj };
        }
    ...
    static Vector3 getLeftMidPoint()
    {
        float bx = left_Final_vertices[0].x, by = left_Final_vertices[0].y, bz = left_Final_vertices[0].z;
        float sx = left_Final_vertices[0].x, sy = left_Final_vertices[0].y, sz = left_Final_vertices[0].z;

        for (int i=1; i<left_Final_vertices.Count; i++)
        {
            if (bx < left_Final_vertices[i].x)
                bx = left_Final_vertices[i].x;

            if (by < left_Final_vertices[i].y)
                by = left_Final_vertices[i].y;

            if (bz < left_Final_vertices[i].z)
                bz = left_Final_vertices[i].z;

            if (sx > left_Final_vertices[i].x)
                sx = left_Final_vertices[i].x;

            if (sy > left_Final_vertices[i].y)
                sy = left_Final_vertices[i].y;

            if (sz > left_Final_vertices[i].z)
                sz = left_Final_vertices[i].z;
        }
        lwide = (bx - sx) * (bz - sz);
        return new Vector3((bx+sx)/2, (by+sy)/2, (bz + sz) / 2);
    }

    static Vector3 getRightMidPoint()
    {
        float bx = right_Final_vertices[0].x, by = right_Final_vertices[0].y, bz = right_Final_vertices[0].z;
        float sx = right_Final_vertices[0].x, sy = right_Final_vertices[0].y, sz = right_Final_vertices[0].z;

        for (int i = 1; i < right_Final_vertices.Count; i++)
        {
            if (bx < right_Final_vertices[i].x)
                bx = right_Final_vertices[i].x;

            if (by < right_Final_vertices[i].y)
                by = right_Final_vertices[i].y;

            if (bz < right_Final_vertices[i].z)
                bz = right_Final_vertices[i].z;

            if (sx > right_Final_vertices[i].x)
                sx = right_Final_vertices[i].x;

            if (sy > right_Final_vertices[i].y)
                sy = right_Final_vertices[i].y;

            if (sz > right_Final_vertices[i].z)
                sz = right_Final_vertices[i].z;
        }
        rwide = (bx - sx) * (bz - sz);
        return new Vector3((bx + sx) / 2, (by + sy) / 2, (bz + sz) / 2);
    }
