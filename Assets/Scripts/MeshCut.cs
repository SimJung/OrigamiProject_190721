...
public class MeshCut : MonoBehaviour
{
    ...
    static float rwide, lwide;
    public static Vector3 leftMidPoint, rightMidPoint;
    // capping stuff
    private static List<Vector3> createdVertexPoints = new List<Vector3>();

    public static GameObject[] Cut(GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Material capMaterial, bool foldCase, Vector3 line, Vector3 tof, float angle)
    {
        ...
        GameObject leftSideObj = new GameObject("leftSide", typeof(MeshFilter), typeof(MeshRenderer), typeof(RectTransform));
        leftSideObj.transform.position = victim_transform.position;
        ...
        GameObject rightSideObj = new GameObject("rightSide", typeof(MeshFilter), typeof(MeshRenderer), typeof(RectTransform));
        ...
        rwide = getRightWide();
        if (lwide <= 0)
            leftSideObj.transform.name = "emptyCut";
        if (rwide <= 0)
            rightSideObj.transform.name = "emptyCut";
        Debug.Log(lwide + "  " + rwide);
        Vector3 lmp = leftSideObj.GetComponent<Rigidbody>().centerOfMass;
        Vector3 rmp = rightSideObj.GetComponent<Rigidbody>().centerOfMass;

        //GameObject lobj = new GameObject("lobj", typeof(RectTransform));
        //GameObject robj = new GameObject("robj", typeof(RectTransform));
        ////lobj.transform.position = leftSideObj.transform.position;
        //lobj.GetComponent<RectTransform>().pivot = anchorPoint;
        //lobj.transform.position = Vector3.zero;
        //leftSideObj.transform.localPosition = Vector3.zero;
        //leftSideObj.transform.parent = lobj.transform;


        //lobj.transform.position += (lobj.transform.rotation * lobj.GetComponent<RectTransform>().pivot);
        //lobj.transform.rotation *= Quaternion.AngleAxis(angle, tof);
        //lobj.transform.position -= (lobj.transform.rotation * lobj.GetComponent<RectTransform>().pivot);

        //leftSideObj.transform.parent = null;


        //robj.transform.position = rightSideObj.transform.position;
        //rightSideObj.transform.localPosition = Vector3.zero;
        //rightSideObj.transform.parent = robj.transform;

        //robj.GetComponent<RectTransform>().pivot = anchorPoint;
        //robj.transform.position += (robj.transform.rotation * robj.GetComponent<RectTransform>().pivot);
        //robj.transform.rotation *= Quaternion.AngleAxis(angle, tof);
        //robj.transform.position -= (robj.transform.rotation * robj.GetComponent<RectTransform>().pivot);
        //rightSideObj.transform.parent = null;


        if (foldCase)
        {
            if (line.x * lmp.x + line.y * lmp.y + line.z > 0)
                return new GameObject[] { rightSideObj, leftSideObj };
            if (line.x * rmp.x + line.y * rmp.y + line.z > 0)
    ...
    static float getLeftWide()
    {
        if (left_Final_vertices.Count <= 0)
            return 0;
        ...
        lwide = (bx - sx) * (by - sy);
        leftMidPoint = new Vector3((bx + sx) / 2, (by + sy) / 2, (bz + sz) / 2);
        return lwide;
    }

    static float getRightWide()
    {
        if (right_Final_vertices.Count <= 0)
            return 0;
        ...
        rwide = (bx - sx) * (by - sy);
        rightMidPoint = new Vector3((bx + sx) / 2, (by + sy) / 2, (bz + sz) / 2);
        return rwide;
    }
}